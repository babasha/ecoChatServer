...
Note: ESP32-C3 = 2.4GHz ONLY (no 5GHz). Hidden SSID: type manually. Dual-band router: disable band steering or create separate 2.4GHz SSID.`,
		...
		"mesh_setup": `For 2+ sensors: first = ROOT (needs WiFi), others = NODE (relay via ESP-NOW, no WiFi needed).
Setup: add sensors via app, they auto-discover mesh. Up to 10 sensors. Place ROOT near router, NODEs within 10m of any node. Self-healing topology.`,
...
Battery wrong: restart sensor, charge fully.`,
		...
		"battery_drain": `Fast drain (normal: 3-6 months at 30min interval):
• 15min=2mo, 30min=4mo, 1hr=6mo — increase interval in Settings
...
5. Server down? Sensor stores data locally, syncs later`,
		...
		"led_error": `LED codes:
GREEN=ready, BLUE=BLE mode, WHITE=charging, YELLOW=firmware update (don't unplug!)
RED slow=low battery, RED fast=error (WiFi/probe/memory — restart), RED solid=hardware fault (charge+restart, if persists contact support)`,
...
func createGetMeshInfoTool() (tool.Tool, error) {
	...
	meshGuides := map[string]string{
		"overview": `Mesh: ROOT (WiFi gateway) + NODEs (relay via ESP-NOW). Up to 10 sensors. Self-healing, auto-routing.
First sensor=ROOT, others auto-join as NODE. Only ROOT needs WiFi. NODEs save battery (no WiFi radio).`,

		"root_node": `ROOT = gateway to cloud. Always on WiFi. Battery ~2mo (vs 4-6 for NODEs).
Place near router. ROOT offline = all sensors lose cloud. Change ROOT: Device Settings → Mesh Role → Promote.`,

		"esp_now": `ESP-NOW: Espressif protocol over 2.4GHz. Range ~10m indoor/~50m outdoor. 1Mbps, <10ms latency, CCMP encrypted.
