...
func DetectProductQuery(message string) *ProductQuery {
	...
	patterns := []struct {
		...
		{`(?:можите|можете|могли\s*бы(?:\s+вы)?)\s+(?:пожалуйста\s+)?(?:рассказать|расказать)\s+(?:пожалуйста\s+)?(?:подробн[ое]е?\s+)?(?:про|о)\s+(.+)`, ProductQuerySearch},
		{`(?:расскажите|раскажите)\s+(?:пожалуйста\s+)?(?:подробн[ое]е?\s+)?(?:про|о)\s+(.+)`, ProductQuerySearch},
		{`подробн[ое]е?\s+(?:пожалуйста\s+)?(?:про|о)\s+(.+)`, ProductQuerySearch},
