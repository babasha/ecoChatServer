...
func (ar *AutoResponder) ProcessMessage(ctx context.Context, chat *models.Chat, msg *models.Message) (*models.Message, error) {
	...
	if escalate {
		...
		if existingEscalation != nil && existingEscalation.ReturnedAt == nil {
			...
			// LLM уже сформировал ответ на языке пользователя, используем его
			// Если LLM не добавил явное сообщение об эскалации, оставляем его ответ как есть
			// clean уже содержит ответ LLM после sanitize()
